...
export class Pawn extends Piece {
    ...
    override move(
        ...
        if (this.color === Color.white) {
            ...
            const bool = Pawn.moveBlack(startSq, endSq, board, undefined, move);
            if (typeof bool === 'boolean') return bool;
        }
        return false;
    }
    ...
    promote(startSq: Square, endSq: Square, board: Board, piece: string): Piece {
        if (this.color === Color.white) {
            const returnPiece = Pawn.moveWhite(startSq, endSq, board, piece);
            ...
            const returnPiece = Pawn.moveBlack(startSq, endSq, board, piece);
            if (!(returnPiece instanceof Piece)) {
                throw new Error('Error promoting');
            } else {
    ...
    private static moveWhite(
        ...
        //last row promote and possibly capture
        else if (
            startSq.getRank === 7 &&
            endSq.getRank === 8 &&
            endSq.getPiece === null
    ...
    static capture(
        ...
        const enPassantEndSqRank = startSq.getPiece?.getColor === Color.white ? 6 : 3;
        ...
        //enpassant is checked before this cause endSq.piece is null on en passant
